...
class PokerRepo {
  ...
  DocumentReference<Map<String, dynamic>> roomRef(String roomId) {
    return db.collection("gameRooms").doc(roomId);
  }

  DocumentReference<Map<String, dynamic>> stateRef(String roomId) {
    return roomRef(roomId).collection("state").doc("current");
  }

  DocumentReference<Map<String, dynamic>> myHandRef(String roomId, String profileId) {
    return roomRef(roomId).collection("privateHands").doc(profileId);
  }

  DocumentReference<Map<String, dynamic>> deckRef(String roomId) {
    return roomRef(roomId).collection("private").doc("host_deck");
  }
  ...
  Future<void> joinRoom({
    ...
    await db.runTransaction((tx) async {
      ...
      if (status == "CLOSED") {
        throw Exception("La sala está cerrada.");
      }

      if (playerIds.contains(profileId)) {
        return;
      }

      if (playerIds.length >= maxPlayers) {
        throw Exception("Sala llena.");
      }
  ...
  Future<void> setReady({
    ...
    await db.runTransaction((tx) async {
      ...
      if (data == null) {
        throw Exception("No existe el estado de la sala.");
      }
      ...
      if (idx < 0) {
        throw Exception("Jugador no encontrado en state/current.");
      }
  ...
  Future<void> rebuyPlayer({
    required String roomId,
    required String profileId,
    int stackAmount = 1000,
  }) async {
    await db.runTransaction((tx) async {
      final stSnap = await tx.get(stateRef(roomId));
      final data = stSnap.data();
      ...
      if (data == null) {
        throw Exception("No existe state/current.");
      }

      final state = PokerState.fromMap(data);

      final idx = state.players.indexWhere((p) => p.profileId == profileId);
      ...
      if (player.stack > 0) {
        throw Exception("Solo puedes reabastecerte si estás sin fichas.");
      }
      ...
      final next = state.replacePlayer(updated).copyWith(
            lastActionText: "$profileId se ha reabastecido a $stackAmount fichas",
          );
  ...
  Future<void> hostStartHand({
    ...
    await syncPlayersFromRoomToState(
      roomId: roomId,
      hostProfileId: hostProfileId,
    );
    ...
    final playersWithStack = current.players.where((p) => p.stack > 0).toList();

    if (playersWithStack.length < 2) {
      throw Exception("Se necesitan al menos 2 jugadores con fichas.");
    }

    final allReady = playersWithStack.every((p) => p.ready);

    if (!allReady) {
      throw Exception("No todos los jugadores con fichas están READY.");
    }
  ...
  Future<void> submitAction({
    ...
    await db.runTransaction((tx) async {
      ...
      while (true) {
        ...
        final bettingRoundFinished =
          PokerEngine.isBettingRoundFinished(next) ||
          (next.turnProfileId.isEmpty &&
              next.currentBet == 0 &&
              next.phase != PokerPhase.waiting &&
              next.phase != PokerPhase.showdown);

        if (bettingRoundFinished) {
          next = PokerEngine.closeBettingRound(next);
  ...
  Future<void> hostAdvanceStreetIfNeeded({
    ...
    if (current.phase == PokerPhase.waiting || current.phase == PokerPhase.showdown) {
      return;
    }

    if (current.turnProfileId.isNotEmpty) {
      return;
    }
    ...
    final advanced = PokerEngine.advanceStreet(current, deck);
    final nextState = advanced.$1;
    final nextDeck = advanced.$2;
