...
class PokerEngine {
  ...
  static StartHandResult startHand(PokerState prev) {
    ...
    final state = prev.copyWith(
      ...
      pot: pot,
      currentBet: players.map((p) => p.betThisStreet).reduce(max),
      board: List.filled(5, "??"),
  ...
  static PokerState applyAction(PokerState s, String actorId, PokerAction action) {
    ...
    if (actor.stack <= 0) {
      throw Exception("Ese jugador está all-in y no puede actuar.");
    }
    ...
    switch (action.type) {
      ...
      case ActionType.call:
        ...
        next = _pay(next, actorId, toCall, "$actorId call ${min(toCall, actor.stack)}");
      ...
      case ActionType.raise:
        ...
        final maxRaiseTo = actor.betThisStreet + actor.stack;

        if (raiseTo > maxRaiseTo) {
          throw Exception("No puedes subir más fichas de las que tienes.");
        }
    ...
    if (alive.length == 1) {
      ...
      final wonAmount = next.pot;

      next = next.replacePlayer(
        winner.copyWith(
          stack: winner.stack + wonAmount,
        ),
      );
      ...
      return next.copyWith(
        ...
        pot: 0,
        winners: [
          {"profileId": winner.profileId, "amount": wonAmount},
        ],
        showdownText: "${winner.profileId} gana $wonAmount porque todos foldearon",
        showdownLines: const [],
        lastActionText: "${winner.profileId} gana $wonAmount porque todos foldearon",
        turnProfileId: "",
        actedThisStreet: const [],
      );
    }
    ...
    if (_isStreetFinished(next)) {
      return closeBettingRound(next);
    }

    final nextTurn = _nextActivePlayerId(next, actorId);

    return next.copyWith(
      turnProfileId: nextTurn,
    );
  }
  ...
  static bool shouldAutoRunoutBoard(PokerState s) {
    ...
    final playersWhoCanAct = alive.where((p) => p.stack > 0).toList();

    if (playersWhoCanAct.isEmpty) {
      return true;
    }

    return _isStreetFinished(s);
  }

  static bool isBettingRoundFinished(PokerState s) {
    return _isStreetFinished(s);
  }

  static PokerState closeBettingRound(PokerState s) {
    final resetPlayers = s.players.map((p) {
      return p.copyWith(
        betThisStreet: 0,
      );
    }).toList();
  ...
  static bool _isStreetFinished(PokerState s) {
    ...
    for (final p in playersWhoCanAct) {
      ...
      if (p.betThisStreet != s.currentBet) {
        return false;
      }
    }
  ...
  static (PokerState, List<String>) advanceStreet(PokerState s, List<String> deck) {
    ...
    String burn() {
      if (d.isEmpty) throw Exception("No quedan cartas para quemar.");
      return d.removeAt(0);
    }

    String draw() {
      if (d.isEmpty) throw Exception("No quedan cartas para repartir.");
      return d.removeAt(0);
    }
  ...
  static String _firstPostflopPlayer(PokerState s) {
    final start = (s.dealerIndex + 1) % s.players.length;
  ...
  static PokerState showdown(PokerState s, Map<String, List<String>> privateHands) {
    ...
    for (final pot in sidePots) {
      final eligible = (pot["eligible"] as List)
          .cast<String>()
          .where(handByPlayer.containsKey)
          .toList();
      ...
      final tied = eligible.where((pid) {
        return handByPlayer[pid]!.compareTo(best) == 0;
      }).toList();

      final amount = pot["amount"] as int;
      final share = amount ~/ tied.length;
      final remainder = amount - (share * tied.length);
    ...
    for (final p in s.players) {
      ...
      final hand = privateHands[p.profileId];
      final hv = handByPlayer[p.profileId];

      if (hand == null || hand.length != 2 || hv == null) continue;

      lines.add(
        "${p.profileId}: ${_displayCard(hand[0])} ${_displayCard(hand[1])} · ${_describeHandValue(hv)}",
      );
    }
  ...
  static List<Map<String, dynamic>> _buildSidePots(List<PlayerState> players) {
    final levels = players.map((p) => p.totalCommitted).where((v) => v > 0).toSet().toList()
      ..sort();
  ...
  static HandValue _eval5(List<_Card> cs) {
    ...
    if (groups.last.value == 2 && groups[groups.length - 2].value == 2) {
      ...
      final kicker = groups.firstWhere((g) => g.value == 1).key;
  ...
  static String _displayCard(String id) {
  final r = id.substring(0, 1);
  final s = id.substring(1, 2);
  ...
  static String _describeHandValue(HandValue hv) {
    String r(int v) {
      return switch (v) {
        14 => "A",
        13 => "K",
    ...
    switch (hv.category) {
      case HandCategory.highCard:
        return "Carta alta ${r(hv.kickers[0])}";
      case HandCategory.onePair:
        return "Pareja de ${r(hv.kickers[0])}";
      case HandCategory.twoPair:
        return "Doble pareja ${r(hv.kickers[0])} y ${r(hv.kickers[1])}";
      case HandCategory.trips:
        return "Trío de ${r(hv.kickers[0])}";
      case HandCategory.straight:
        return "Escalera hasta ${r(hv.kickers[0])}";
      case HandCategory.flush:
        return "Color al ${r(hv.kickers[0])}";
      case HandCategory.fullHouse:
        return "Full house de ${r(hv.kickers[0])} sobre ${r(hv.kickers[1])}";
      case HandCategory.quads:
        return "Póker de ${r(hv.kickers[0])}";
      case HandCategory.straightFlush:
        return "Escalera de color hasta ${r(hv.kickers[0])}";
    }
  }
}